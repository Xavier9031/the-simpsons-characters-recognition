...
parser.add_argument('--data_test_dir', type=str, default='test-final/test-final')
...
#/mnt/htchang/ML/machine-learning-2023nycu-classification/simpsons/checkpoint-58150
model = ViTForImageClassification.from_pretrained(args.model_dir,id2label=id2label,label2id=label2id)
...
for image_file in image_files:
    ...
    with torch.no_grad(): 
        ...
        target_image_pred_probs = torch.softmax(target_image_pred.logits, dim=1)
        target_image_pred_label = torch.argmax(target_image_pred_probs, dim=1)
        label_index.append(target_image_pred_label.item())
        label_id.append(image_file)
        print(f"Image {image_file} is predicted as class: {target_image_pred_label.item()}, probability: {max(target_image_pred_probs[0])}")

df = pd.DataFrame({'id': image_files, 'character': label_index})
df['id'] = df['id'].apply(lambda x: x.split('.')[0])

df['character'] = df['character'].map(id2label)

# 保存为csv文件
df.to_csv(args.submission_name, index=False)